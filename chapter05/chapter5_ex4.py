...
def id_generator(start_id):
    ...
    Will raise exception if given start_id not int between 0 - MAX_ID_NUMBER.

    :param start_id: Minimal number for iterator's first value.
    :type start_id: int
    :return: ID numbers generator.
    :rtype: generator
    """
    if not isinstance(start_id, int) or start_id < 0 or MAX_ID_NUMBER < start_id:
        raise ValueError(f'Initial ID number mast be between 0 and '
                         f'{MAX_ID_NUMBER} (got: {start_id}).')
...
def main():
    """Use IDIterator and id_generator to print 10 IDs, start at: 123456780"""
    print('--- IDIterator ---')
    try:
    ...
    try:
        ...
        print(f'Failed to use id_generator - {e}')
